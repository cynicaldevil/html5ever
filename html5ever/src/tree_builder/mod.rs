          ...
          Sink: TreeSink<Handle=Handle> + Sendable,
{
    ...
    fn get_sendable(&self) -> Self::SendableSelf {
        ...
        while let Some(elem) = self.pending_table_text.iter().next() {
        ...
        while let Some(elem) = self.active_formatting.iter().next() {
            let sendable_elem = match *elem {
                FormatEntry::Element(ref handle, ref tag) => {
                    let mut sendable_attrs = vec!();
                    while let Some(ref attr) = tag.attrs.iter().next() {
    ...
    fn get_self_from_sendable(sendable_self: Self::SendableSelf) -> Self {
        ...
        while let Some(elem) = sendable_self.pending_table_text.iter().next() {
        ...
        while let Some(sendable_elem) = sendable_self.active_formatting.iter().next() {
            let elem = match sendable_elem.clone() {
                SendableFormatEntry::Element {
                    ...
                    let mut attrs = vec!();
                    while let Some(attr) = tag_attrs.iter().next() {
